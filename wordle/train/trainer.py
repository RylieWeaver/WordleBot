...
class Trainer:
    ...
    def _run_eval(self, epoch: int):
        ...
        self.logger.log(
            ...
            f"Policy Gradient: {test_loss_components['policy_gradient']:.6f} | Entropy Loss: {test_loss_components['entropy']:.6f} | "
            f"KL Reg Loss: {test_loss_components['kl_reg']:.6f} | KL Guide Loss: {test_loss_components['kl_guide']:.6f} | "
            f"Adv Mean: {test_loss_components['advantage_mean']:.6f} | Adv Std: {test_loss_components['advantage_std']:.6f} | "
            f"Q Search Mean: {test_loss_components['q_search_mean']:.6f} | V Search Mean: {test_loss_components['v_search_mean']:.6f}"
        )
    ...
    def _run_epoch(self, epoch: int):
        ...
        for i in range(self.cfg.rollout_size):
            ...
            self.logger.log(
                f"[Epoch {epoch}][{desc}] Loss: {rollout_loss:.6f} | Policy Gradient: {rollout_loss_components['policy_gradient']:.6f} | "
                f"Entropy Loss: {rollout_loss_components['entropy']:.6f} | KL Reg Loss: {rollout_loss_components['kl_reg']:.6f} | "
                f"KL Guide Loss: {rollout_loss_components['kl_guide']:.6f} | Adv Mean: {rollout_loss_components['advantage_mean']:.6f} | "
                f"Adv Std: {rollout_loss_components['advantage_std']:.6f} | Q Search Mean: {rollout_loss_components['q_search_mean']:.6f} | "
                f"V Search Mean: {rollout_loss_components['v_search_mean']:.6f}"
            )

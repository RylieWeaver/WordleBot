...
class Trainer:
    def __init__(self, cfg, model, device: Union[str, torch.device] = "cpu") -> None:
        # Read vars
        self.cfg = cfg
    ...
    """
    def __reinit__(self):
        self.__init__(self.cfg, self.model, self.device)
        ...
        for attempt in range(1, self.cfg.max_batch_attempts + 1):
            try:
                ...
                batch_loss, batch_loss_components = self.loss.inc_loss(
                    states, actions, responses, probs
                )

                # Measure grad norms
                if measure_grad_norms:
                    self.loss.measure_grad_norms(self.model, states, actions, responses, probs, alpha, temp)
    ...
    def measure_grad_norms(self, episodes_batch, alpha=None, temp=None):
        ...
        responses = self._float32_tree(move_to(responses, self.device))
        probs = self._float32_tree(move_to(probs, self.device))
        self.loss.measure_grad_norms(self.model, states, actions, responses, probs)
    ...
    def _run_eval(self, epoch: int):
        ...
        self.logger.log(
            ...
            f"Search KL: {test_loss_components['search_kl']:.6f}"
        )
    ...
    def _run_epoch(self, epoch: int):
        ...
        for i in range(self.cfg.rollout_size):
            ...
            self.logger.log(
                f"[Epoch {epoch}][{desc}] Loss: {rollout_loss:.6f} | "
                f"Search KL: {rollout_loss_components['search_kl']:.6f}"
            )

...
class EnumLossComponent:
    ...
    Q_SEARCH_MEAN = "q_search_mean"
    V_SEARCH_MEAN = "v_search_mean"
...
class WordleLoss:
    ...
    def make_loss(self):
        return 0.0, {
            ...
            EnumLossComponent.ADVANTAGE_MEAN: 0.0,
            EnumLossComponent.ADVANTAGE_STD: 0.0,
            EnumLossComponent.Q_SEARCH_MEAN: 0.0,
            EnumLossComponent.V_SEARCH_MEAN: 0.0,
        }
    ...
    def inc_loss(self, states, actions, responses, probs, ref_probs=None):
        ...
        if "advantage_mean" in responses:
            batch_loss_components[EnumLossComponent.ADVANTAGE_MEAN] = to_float(responses["advantage_mean"])
        if "advantage_std" in responses:
            batch_loss_components[EnumLossComponent.ADVANTAGE_STD] = to_float(responses["advantage_std"])
        if "q_search_mean" in responses:
            batch_loss_components[EnumLossComponent.Q_SEARCH_MEAN] = to_float(responses["q_search_mean"])
        if "v_search_mean" in responses:
            batch_loss_components[EnumLossComponent.V_SEARCH_MEAN] = to_float(responses["v_search_mean"])

        ...
        try:
            with torch.no_grad():
                for turn in range(G):
                    ...
                    adv = q_search - v_search
                    q_values.append(q_search)
                    v_values.append(v_search)
                    advantages.append(adv)
        finally:
            model.train(was_training)

        q_values = torch.stack(q_values, dim=1)
        v_values = torch.stack(v_values, dim=1)
        advantages = torch.stack(advantages, dim=1)
        active = states["active_mask"][:, :-1, ...].bool()
        if active.any():
            active_adv = advantages[active]
            advantage_mean = active_adv.mean()
            advantage_std = active_adv.std(unbiased=False)
            q_search_mean = q_values[active].mean()
            v_search_mean = v_values[active].mean()
        else:
            device = states["active_mask"].device
            advantage_mean = torch.zeros((), dtype=self.fp_dtype, device=device)
            advantage_std = torch.zeros((), dtype=self.fp_dtype, device=device)
            q_search_mean = torch.zeros((), dtype=self.fp_dtype, device=device)
            v_search_mean = torch.zeros((), dtype=self.fp_dtype, device=device)

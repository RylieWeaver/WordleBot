...
from wordle.environment import sample_possible_targets, make_probs, select_actions, select_actions_search, simulate_actions
...
def collect_episodes(
    ...
    guide_probs_batch = torch.zeros((batch_size, max_guesses, total_vocab_size), dtype=torch.float32, device=device)
    ...
    for t in range(max_guesses):
        ...
        # Select actions
        if search:
            probs, _, guide_probs, valid_mask, guess_idx, guess_idx_onehot, guess_tensor = select_actions_search(
                actor_critic_net,
                alphabet_states,
                guess_states,
                alphabet_entropy,
            ...
            probs, guide_probs, valid_mask, guess_idx, guess_idx_onehot, guess_tensor = select_actions(
                actor_critic_net,
                alphabet_states,
                guess_states,
                total_vocab,
        ...
        _, _, _, _, cand_values, cand_rewards, _ = simulate_actions(
            actor_critic_net,
            alphabet_states.unsqueeze(1).expand(-1, m, -1, -1),
            guess_states.unsqueeze(1).expand(-1, m, -1),
            alphabet_entropy.unsqueeze(1).expand(-1, m),
            target_mask.unsqueeze(1).expand(-1, m, -1),
            target_vocab_states.unsqueeze(0).expand(m, -1, -1, -1),
        ...
        # Store
        probs_batch[:, t, :] = probs
        guide_probs_batch[:, t, :] = guide_probs
...
def process_episodes(
    ...
    for t in reversed(range(max_guesses)):
        ...
        advantage_blend_factor = 0.1
        blended_rewards = reward_blend_factor*expected_rewards_batch[:, t] + (1-reward_blend_factor)*rewards_batch[:, t]
        blended_values = advantage_blend_factor*expected_values_batch[:, t] + (1-advantage_blend_factor)*values_batch[:, t+1]
